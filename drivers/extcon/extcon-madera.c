...
static void madera_handle_usbc_audio(struct madera_extcon *info, bool connected)
{
	dev_err(info->dev, "Type-C Audio Adapter %sconnected\n",
					connected ? "" : "dis");
	if (info->usbc_det_gpio[0] != NULL && info->usbc_det_gpio[1] != NULL) {
					      ...
					      struct fwnode_handle *node)
{
	...
	if (info->usbc_headset_support) {
		...
		info->usbc_det_gpio[0] = devm_fwnode_get_gpiod_from_child(info->dev,
								"cirrus,usbsel",
								node,
								GPIOD_OUT_LOW,
								"cirrus,usbsel");
		if (IS_ERR(info->usbc_det_gpio[0]))
			info->usbc_det_gpio[0] = NULL;
		else {
		...
		info->usbc_det_gpio[1] = devm_fwnode_get_gpiod_from_child(info->dev,
								"cirrus,hsdet",
								node,
								GPIOD_OUT_LOW,
								"cirrus,hsdet");
		if (IS_ERR(info->usbc_det_gpio[1]))
			info->usbc_det_gpio[1] = NULL;
		else {
